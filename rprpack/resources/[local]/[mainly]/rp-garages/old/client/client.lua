...
local function spawnVehicle(data, spawn, price, callback_vehicle, vehicle)
    lib.requestModel(data.vehicle.model)
    TriggerServerEvent('esx_garages:SpawnVehicle', data.vehicle.model, data.vehicle.plate, vector3(spawn.x, spawn.y, spawn.z-1), type(spawn) == 'vector4' and spawn.w or spawn.h, price)
    --TriggerServerEvent('sv-addkeys', data.vehicle.plate)
end

RegisterNetEvent('dajklucze', function(data)
    TriggerServerEvent('sv-addkeys', data.vehicle.plate)
end)
...
local function ImpoundBlips(coords, type, label, blipOptions)
    ...
    AddTextComponentString("Holownik")
...
local function GarageBlips(coords, type, label, job, blipOptions)
    ...
    SetBlipAsShortRange(blip, true)
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentString(Config.SplitGarages == true and label or "Garaż")
    EndTextCommandSetBlipName(blip)
end
...
local function JobGarageBlip(garage)
    ...
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentString(Config.SplitGarages == true and garage.label or "Garaż")
...
for k, v in pairs(Config.Garages) do
    ...
    exports['qtarget']:AddTargetModel({v.ped or Config.DefaultGaragePed}, {
        options = {
            {
                event = "esx_garages:GetOwnedVehicles",
...
exports['qtarget']:AddTargetModel(impoundPeds, {
    options = {
        {
            event = 'esx_garages:GetImpoundedVehicles',
            icon = "fas fa-key",
...
AddStateBagChangeHandler('vehicleData', nil, function(bagName, key, value, _unused, replicated)
    ...
    TriggerServerEvent('esx_garages:ChangeStored', value.plate)
    Entity(vehicle).state:set('vehicleData', nil, true)
end)

RegisterNetEvent('esx_garages:GetImpoundedVehicles', function()
    local vehicles = lib.callback.await('esx_garages:GetImpound', false, currentImpound.type)
    local options = {}
    ...
    for i = 1, #vehicles do
        ...
        options[i] = {
            title = vehicleTitle,
            event = 'esx_garages:ImpoundVehicleMenu',
            arrow = true,
    ...
    lib.showContext('esx_garages:ImpoundMenu')
end)

RegisterNetEvent('esx_garages:GetOwnedVehicles', function()
    local vehicles = lib.callback.await('esx_garages:GetVehicles', false, currentGarage.type, currentGarage.job)
    local options = {}

    if not vehicles then
        lib.registerContext({
            id = 'esx_garages:GarageMenu',
            title = Config.SplitGarages == true and currentGarage.label or 'Garaż',
            options = {
    ...
    for i = 1, #vehicles do
        ...
        options[i] = {
            title = vehicleTitle,
            event = stored and 'esx_garages:VehicleMenu' or nil,
    ...
    lib.showContext('esx_garages:GarageMenu')
end)

RegisterNetEvent('esx_garages:ImpoundVehicleMenu', function(data)
    lib.registerContext({
        id = 'esx_garages:ImpoundVehicleMenu',
        title = data.name,
        ...
        options = {
            [Locale('take_out_vehicle_impound')] = {
                ...
                event = 'esx_garages:RequestVehicle',
                args = {
                    vehicle = data.vehicle,
                    price = data.price,
                    type = 'impound'
    ...
    lib.showContext('esx_garages:ImpoundVehicleMenu')
end)

RegisterNetEvent('esx_garages:VehicleMenu', function(data)
    lib.registerContext({
    ...
    lib.showContext('esx_garages:VehicleMenu')
end)

RegisterNetEvent('esx_garages:RequestVehicle', function(data)
    local spawn = nil
...
RegisterNetEvent('esx_garages:StoreVehicle', function(target)
    local vehicle = target.entity
    local vehPlate = GetVehicleNumberPlateText(vehicle)
    local vehProps = lib.getVehicleProperties(vehicle)

    local doesOwn, isInvalid = lib.callback.await('esx_garages:CheckOwnership', false, vehPlate, vehProps.model, currentGarage)
    if doesOwn then
       if isInvalid then return ESX.ShowNotification(Locale('garage_cant_store')) end
        TriggerServerEvent('esx_garages:SaveVehicle', vehProps, vehPlate, VehToNet(vehicle), currentGarage.zone.name)
        TriggerServerEvent('sv-removekeys', vehPlate)
    else
