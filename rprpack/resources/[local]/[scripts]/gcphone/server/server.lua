...
function getPhoneRandomNumber()
    ...
    local numBase1 = math.random(0,99)
...
AddEventHandler('gcPhone:useSimCard', function(source, identifier)
    ...
    repeat
        ...
        local id = getIdentifierByPhoneNumber(phone_number)
    until id == nil
    MySQL.Async.insert("UPDATE users SET phone_number = @phone_number WHERE identifier = @identifier", { 
        ...
        TriggerClientEvent("gcPhone:phone_number", _source, num)
...
function getOrGeneratePhoneNumber (identifier, cb)
    ...
    local phone_number = getNumberPhone(identifier)
    if phone_number == '0' or phone_number == nil then
        repeat
            phone_number = getPhoneRandomNumber()
            local id = getIdentifierByPhoneNumber(phone_number)
        until id == nil
...
AddEventHandler('esx:playerLoaded',function(playerId, xPlayer)
	...
	getOrGeneratePhoneNumber(identifier, function (phone_number)
        TriggerClientEvent('gcPhone:phone_number', sourcePlayer, phone_number)
        TriggerClientEvent('gcPhone:contactList', sourcePlayer, getContacts(identifier))
        TriggerClientEvent('gcPhone:allMessage', sourcePlayer, getMessages(identifier))
...
AddEventHandler('gcPhone:allUpdate', function()
    ...
    local identifier = xPlayer.identifier
    local num = getNumberPhone(identifier)
    TriggerClientEvent("gcPhone:phone_number", sourcePlayer, num)
    TriggerClientEvent("gcPhone:contactList", sourcePlayer, getContacts(identifier))
    TriggerClientEvent("gcPhone:allMessage", sourcePlayer, getMessages(identifier))
    TriggerClientEvent('gcPhone:getBourse', sourcePlayer, getBourse())
    sendHistoriqueCall(sourcePlayer, num)
