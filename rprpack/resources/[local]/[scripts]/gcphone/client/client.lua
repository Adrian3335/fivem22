...
AddEventHandler("gcPhone:forceOpenPhone", function(_phone_number)
...
--  Events
--====================================================================================
RegisterNetEvent("gcPhone:phone_number")
AddEventHandler("gcPhone:phone_number", function(_phone_number)
  phone_number = _phone_number
  SendNUIMessage({event = 'updatephone_number', phone_number = phone_number})
end)
...
RegisterNUICallback('takePhoto', function(data, cb)
  ...
  if hasFocus == true then
    ...
    elseif IsControlJustPressed(1, 176) then -- TAKE.. PIC
        ...
        DestroyMobilePhone()
        CellCamActivate(false, false)
        cb(json.encode({ url = data.files[1].url }))   
        cb(json.encode({ url = data.url }))
      end)
      takePhoto = false
		end
