...
RegisterCommand("dowod", function(source)
    ...
    TriggerClientEvent("dbl_documents:playAnim", source, 'dowod')
    Wait(2000)

    local coords = GetEntityCoords(GetPlayerPed(source))
    for _, id in pairs(ESX.GetPlayers()) do
        if #(coords - GetEntityCoords(GetPlayerPed(id))) < 7 then
            TriggerClientEvent('dbl_documents:showDocument', id, 'dowod', playerData)
        end
    end
end)
...
RegisterCommand("wizytowka", function(source)
    ...
    TriggerClientEvent("dbl_documents:playAnim", source, 'wizytowka')
    Wait(2000)

    local coords = GetEntityCoords(GetPlayerPed(source))
    for _, id in pairs(ESX.GetPlayers()) do
        if #(coords - GetEntityCoords(GetPlayerPed(id))) < 7 then
            TriggerClientEvent('dbl_documents:showDocument', id, 'wizytowka', playerData)
        end
    end
end) 
...
RegisterCommand("odznaka", function(source)
    ...
    if job.name == 'police' or job.name == 'lssd' then
        ...
        TriggerClientEvent("dbl_documents:playAnim", source, 'odznaka')
        Wait(1500)
        ...
        for _, id in pairs(ESX.GetPlayers()) do
            if #(coords - GetEntityCoords(GetPlayerPed(id))) < 7 then
                if job.name == 'police' then
                    TriggerClientEvent('dbl_documents:showDocument', id, 'odznaka', playerData)
                else
                    TriggerClientEvent('dbl_documents:showDocument', id, 'odznakaLSSD', playerData)
                end
            end
        end
    else
...
RegisterCommand("legitymacja", function(source)
    ...
    for _, id in pairs(ESX.GetPlayers()) do
        if #(coords - GetEntityCoords(GetPlayerPed(id))) < 7 then
            TriggerClientEvent('dbl_documents:showDocument', id, job.name, playerData)
        end
    end
end) 