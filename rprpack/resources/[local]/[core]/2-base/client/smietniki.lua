    ...
    while true do
        ...
        if inTrash then
            ...
            if DoesEntityExist(dumpster) or not IsPedDeadOrDying(PlayerPedId()) or not IsPedFatallyInjured(PlayerPedId()) then
                ...
                exports['very-hnotify']:ShowHelpNotification('Naciśni ~INPUT_PICKUP~ aby wyjść ze śmietnika.')
    ...
    while true do
        ...
        if canHide then
            ...
            for i = 1, #dumpsters do
                ...
                if dist < 1.8 then
                    if not inTrash then
                        exports['very-hnotify']:ShowHelpNotification('Naciśni ~INPUT_PICKUP~ aby schować się w śmietniku.')
                            ...
                            if DoesEntityExist(ped) then
                                if not IsEntityAttached(ped) or GetDistanceBetweenCoords(GetEntityCoords(ped), GetEntityCoords(PlayerPedId()), true) >= 5.0 then
                                    ...
                                    TriggerEvent('chat:addMessage', {
                                        color = { 255, 0, 0},
                                        multiline = true,
                                        args = {"SYSTEM", "Już ktoś siedzi w śmietniku!"}
                                      })
                                end
                            end
                        end
