...
local volumes = {
	...
	['radio'] = GetConvarInt('voice_defaultRadioVolume', 70) / 100,
	['phone'] = GetConvarInt('voice_defaultPhoneVolume', 70) / 100,
}
...
CreateThread(function()
	while true do
		...
		if display then
			...
			drwRct(0.085, 0.985, 0.0705, 0.0125, 0, 0, 0, 120)
			drwRct(0.0855, 0.985, size, 0.01, color[1], color[2], color[3], 70)

			if callChannel ~= 0 then
				DrawSprite('mpleaderboard', 'leaderboard_friends_icon', 0.148, 0.991, 0.012, 0.022, 0.0, 255, 255, 255, 255)

				if playerCallSpeaker then
					DrawSprite('mpleaderboard', 'leaderboard_plus_icon', 0.138, 0.991, 0.012, 0.022, 0.0, 255, 255, 255, 255)
				end
			elseif playerRadioActive then
				DrawSprite('mpleaderboard', 'leaderboard_audio_3', 0.148, 0.991, 0.012, 0.022, 0.0, 255, 255, 255, 255)
			end
		else
			Citizen.Wait(1000)
		end
...
RegisterNetEvent('kariee-voice:setDisplay')
AddEventHandler('kariee-voice:setDisplay', function(val)
	barVisible = tonumber(val) ~= 0.0
	display = tonumber(val) ~= 0.0
end)
...
RegisterCommand('vol', function(_, args)
	if not args[1] then
		TriggerEvent("esx:showNotification", 'Podaj docelową głośność')
		return
	end	
	if not args[2] then
		TriggerEvent("esx:showNotification", 'Podaj typ voice w którym chcesz zmienić głośność, prawidłowe: radio, phone')
		return
	end	
	local volume = tonumber(args[1])
	local typee = args[2]
	if typee == "radio" or typee == "phone" then
		TriggerEvent("esx:showNotification", '~Zły typ voice! Prawidłowe: radio, phone')
		return
	end	
	if volume then
...
RegisterCommand('radioeffect', function(_, args)
	...
	if tglRadioEffect then
		TriggerEvent("esx:showNotification", 'Włączyłeś efekt radia')
	else
		TriggerEvent("esx:showNotification", 'Wyłączyłeś efekt radia')
	end
end)
