...
function isDead()
	return IsEntityPlayingAnim(PlayerPedId(), "dead", "dead_a", 3)
end
	...
	if not radioPressed and radioEnabled then
		if radioChannel > 0 then
			...
			if GetConvarInt('voice_enableRadioAnim', 0) == 1 and not (GetConvarInt('voice_disableVehicleRadioAnim', 0) == 1 and IsPedInAnyVehicle(PlayerPedId(), false)) then
				if PlayerData.job.name == "police" or PlayerData.job.name == "sheriff" or PlayerData.job.name == "ambulance" or PlayerData.job.name == "mechanik2" or PlayerData.job.name == "gheneraugarage" then
				...
				TriggerServerEvent("exilerp:addTalking")
			end
			CreateThread(function()
...
RegisterCommand('-radiotalk', function()
	...
	if radioChannel > 0 or radioEnabled and radioPressed then
		...
		if GetConvarInt('voice_enableRadioAnim', 0) == 1 then
			if PlayerData.job.name == "police" or PlayerData.job.name == "ambulance" or PlayerData.job.name == "uwu" then
				ClearPedTasks(PlayerPedId())
				TriggerServerEvent("exilerp:walkieTalkie", false)
			else
				StopAnimTask(PlayerPedId(), "random@arrests", "generic_radio_enter", -4.0)
			end	
			TriggerServerEvent("exilerp:stopTalking")
		end
