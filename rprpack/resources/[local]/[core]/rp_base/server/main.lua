ESX.RegisterServerCallback('dbl_base:GetCurrentPlayers', function(source, cb)
    cb(#GetPlayers())
end)
local identifiers = {discord = "", steam = "", license = "", ip = "", live = "", xbl = ""}
local dono = "NeedRP | LogSystem"
local communtiylogo = "https://cdn.discordapp.com/attachments/1072605585498984563/1129173878179123280/needrp_logo_nowe.png"  
...
CreateThread(function()
    while true do
        ...
        TriggerClientEvent("dbl_base:UpdateTime", -1, hour, minute)
    end
end)
...
ESX.RegisterUsableItem('dildo', function(source)
	...
	TriggerClientEvent("dbl_use:dildo", source)
end)

ESX.RegisterUsableItem('wheelchair', function(source)
	local xPlayer = ESX.GetPlayerFromId() 	
	print("JEBAC KRISSA")
	xPlayer.triggerEvent("dbl_use:wheelchair")
end)

RegisterNetEvent("dbl_watercooler:buywater", function()
	local xPlayer = ESX.GetPlayerFromId(source)
	if xPlayer.getMoney() >= 25 then 
		xPlayer.removeAccountMoney('money', 25)
		xPlayer.addInventoryItem('water', 1)
		exports["dbl_notify"]:notify(source, "success", "Powiadomienie", "Zakupiłeś wodę.")
	else 
		exports["dbl_notify"]:notify(source, "failure", "Powiadomienie", "Nie posiadasz tyle gotówki!")
	end
end)
...
ESX.RegisterUsableItem('wheelchair', function(playerId)
    ...
    Wait(250)
    xPlayer.triggerEvent('dbl:wozek')
end)

RegisterServerEvent('dbl:dodajwozek', function()
    local xPlayer = ESX.GetPlayerFromId(source)
