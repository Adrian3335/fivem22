...
Citizen.CreateThread(function()
    ...
    --                 TriggerServerEvent("SLUSHRP:avviarapina")
    --             else
...
------------------------------------PED MODEL--------------------------------------
    ...
    exports['qtarget']:AddTargetEntity(ped_lester, {
        options = {
            {
                label = 'Weź zlecenie',
                icon = 'fas fa-house',
...
RegisterNetEvent("SLUSHRP:cercacasa")
AddEventHandler("SLUSHRP:cercacasa", function()
    ...
    HouseRobb = false
    exports['qtarget']:AddBoxZone('house', vec3(blip_posizione.x, blip_posizione.y, blip_posizione.z), 1.0, 1.0, {
        name = 'house',
        heading = 0.0,
        debugPoly = false,
        minZ = blip_posizione.z - 1,
        maxZ = blip_posizione.z + 1
    }, {
        options = {
            {
                label = 'Obrabuj dom',
                    ...
                    if entratincasa == false then
                        if GetVehiclePedIsIn(pedid, false) ~= 0 then
                            ...
                            if HouseRobb == false then
                                ESX.TriggerServerCallback('SLUSHRP:checkitem', function(lockpick)
                                    if lockpick >= 1 then
                                        entratincasa = true
                                        FreezeEntityPosition(pedid, true)
	...
	local ATMRobbery = {
		...
		id = randomId(),
		priority = 1,
		title = "Rabunek Mieszkania",
		duration = 5500,
		blipname = "[DISPATCH] Rabunek Mieszkania",
		color = 1,
		sprite = 500,
		fadeOut = 30,
		position = {
	...
	TriggerServerEvent("dispatch:svNotify", ATM2Robbery)
                                        ClearPedTasks(pedid)
                                        FreezeEntityPosition(pedid, false)
                                        TriggerServerEvent("SLUSHRP:toglitem")
...
function CreaOggettiCasa(pos_casa)
    ...
    Oggettinellafottutacasa = {
        [1] = { id = 1, x = pos_casa.x+2.6, y = pos_casa.y+2, z = pos_casa.z+2.1, h = "w książce"},
        [2] = { id = 2, x = pos_casa.x+1.5, y = pos_casa.y-3.8, z = pos_casa.z+2.1, h = "w lodówce"},
        [3] = { id = 3, x = pos_casa.x+7.6, y = pos_casa.y+4.1, z = pos_casa.z+2.1, h = "w szafie"},
        [4] = { id = 4, x = pos_casa.x-5, y = pos_casa.y+1, z = pos_casa.z+2.1, h = "w szufladach"},
        [5] = { id = 5, x = pos_casa.x-1.9, y = pos_casa.y-6.5, z = pos_casa.z+2.1, h = "na półce"},
    }
    

    for k,v in pairs(Oggettinellafottutacasa) do
        exports['qtarget']:AddBoxZone('search'..v.id, vec3(v.x, v.y, v.z), 1.0, 1.0, {
            name = 'search'..v.id,
            heading = 0.0,
            debugPoly = false,
            minZ = v.z - 1,
                    ...
                    action = function()
                        ...
                        if v.id == 1 then
                            if cercaogg1 == false then
                                TriggerServerEvent("SLUSHRP:cercaoggetto", posiz)
                                cercaogg1 = true
                            else
...
function EsciDaCasa(pos_casa)
    local caz = Config.Marker.house_robbery
    exports['qtarget']:AddBoxZone('exit_house', vec3(pos_casa.x+3.7, pos_casa.y-15.6, pos_casa.z+2.1), 1.0, 1.0, {
        name = 'exit_house',
        heading = 0.0,
        debugPoly = false,
        minZ = pos_casa.z - 4,
    ...
    for k,v in pairs(Oggettinellafottutacasa) do
        exports['qtarget']:RemoveZone('search'..v.id)
    end
end
...
function DistruggiCasa(pos_casa)
    ...
    TriggerEvent("SLUSHRP:stoppa")
    local giocatore = GetEntityCoords(PlayerPedId(-1))
    --TriggerClientEvent("SLUSHRP:allertapolice")
end
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if Nui then
                ...
                TriggerEvent("SLUSHRP:avvia", math.ceil(disturbance))
...
-- AddEventHandler('SLUSHRP:allertapolice', function()
--     local coords = GetEntityCoords(GetPlayerPed(-1))
-- 	local ATMRobbery = {
-- 		code = "10-90",
-- 		street = exports['mkwy_dispatch']:GetStreetAndZone(),
-- 		id = exports['mkwy_dispatch']:randomId(),
-- 		priority = 1,
-- 		title = "Rabunek Mieszkania",
-- 		duration = 5500,
-- 		blipname = "[DISPATCH] Rabunek Mieszkania",
...
-- RegisterNetEvent('SLUSHRP:clientpolice')
-- AddEventHandler('SLUSHRP:clientpolice', function(target, coords, hour)
-- 	local s1, s2 = Citizen.InvokeNative(0x2EB41072B4C1E4C0, coords.x, coords.y, coords.z, Citizen.PointerValueInt(), Citizen.PointerValueInt())
-- 	exports["mkwy-Powiadomienia"]:policeNotify({
-- 		playerId = target,
-- 		coords = coords,
-- 		textColor = "#fff",
-- 		bgColor = "#06a5ff",
