...
AddEventHandler('esx_jobChat:911', function(targetCoords, msg, streetName, emergency)
	...
	if emergency == '911' then
		messageFull = {
			template = '<div style="padding: 8px; margin: 8px; background-color: rgba(255, 0, 0, 0.4); border-radius: 25px;"><i class="fas fa-bell"style="font-size:15px"></i> [LSPD] : <br> Imię i nazwisko: {0}  <br> Lokalizacja : {1}  <br> Wiadomość: {2}</font></i></b></div>',
