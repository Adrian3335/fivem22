...
RegisterServerEvent('qf_mdt:addNote', function(title, content)
    ...
    MySQL.query('SELECT annid FROM qf_mdt_notes ORDER BY id DESC LIMIT 1', function(lastIndex)
        local lI = 1
        if #lastIndex > 0 then
            lI = lastIndex[1].annid + 1
        end
            
        MySQL.insert('INSERT INTO qf_mdt_notes (identifier, title, content, time, annid) VALUES (?, ?, ?, ?, ?)', {xPlayer.identifier, title, content, os.time(), lI})
    end)

    Wait(500)

    local notes = MySQL.query.await('SELECT title, content, annid FROM qf_mdt_notes WHERE identifier = ? ORDER BY time', {xPlayer.identifier})
...
RegisterServerEvent('qf_mdt:deleteNote', function(data)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    MySQL.update('DELETE FROM qf_mdt_notes WHERE annid = ? AND identifier = ?', {data, xPlayer.identifier})

    local notes = MySQL.query.await('SELECT title, content, annid FROM qf_mdt_notes WHERE identifier = ? ORDER BY time', {xPlayer.identifier})
    TriggerClientEvent('qf_mdt:updatePersonalNotes', _source, notes)
end)

function updateLastNotes(src)
    SERVER.lastNotes = {
        vehicles = {},
        citizen = {}
    }

    local getLastCitizenNotes = MySQL.query.await('SELECT * FROM qf_mdt_citizen_notes ORDER BY date DESC LIMIT 5')
    local getLastVehicleNotes = MySQL.query.await('SELECT * FROM qf_mdt_vehicle_notes ORDER BY date DESC LIMIT 5')
    
    for i = 1, #getLastCitizenNotes, 1 do
        local name = MySQL.scalar.await('SELECT CONCAT(firstname, " ", lastname) FROM users WHERE identifier = ?', {getLastCitizenNotes[i].identifier})


        table.insert(SERVER.lastNotes.citizen, {
            name = name or "",
            date = getLastCitizenNotes[i].date,
            reason = getLastCitizenNotes[i].reason,
            officer = getLastCitizenNotes[i].officer
        })
    end
...
--
    ...
    --        model = newModel,
       ...
       --     reason = getLastVehicleNotes[i].note,
        ...
        --    officer = getLastVehicleNotes[i].officer
       -- })
   -- end

    TriggerClientEvent('qf_mdt:updateLastNotes', -1, SERVER.lastNotes)
end
...
ESX.RegisterServerCallback('qf_mdt:getMDTData', function(source, cb)
    ...
    updateLastNotes(source)

    cb(data, notes, SERVER.annoucements, SERVER.officers)
end)
...
ESX.RegisterServerCallback('qf_mdt:searchVehicle', function(source, cb, value)
    MySQL.query('SELECT users.identifier, owned_vehicles.plate, owned_vehicles.vehicle, CONCAT(users.firstname, " ", users.lastname) as owner FROM owned_vehicles INNER JOIN users ON users.identifier = owned_vehicles.owner WHERE owned_vehicles.plate = ?', {value:upper()}, function(results)
        for i=1, #results, 1 do
            local x = json.decode(results[i].vehicle).model
            results[i].model = getVehicleModel(source, x)
        end
        cb(results)
    end)
end)
...
RegisterServerEvent('qf_mdt:addVehicleNote', function(data)
    ...
    local officer = EDITABLE.GetBadge(xPlayer) .. " " .. xPlayer.getName()
    MySQL.insert('INSERT INTO qf_mdt_vehicle_notes (plate, date, note, officer) VALUES (?, ?, ?, ?)', {data.plate:upper(), os.time(), data.note, officer})
...
RegisterServerEvent('qf_mdt:addCitizenNote', function(data)
    ...
    local officer = EDITABLE.GetBadge(xPlayer) .. " " .. xPlayer.getName()
    MySQL.insert('INSERT INTO qf_mdt_citizen_notes (identifier, date, reason, officer) VALUES (?, ?, ?, ?)', {data.identifier, os.time(), data.note, officer})
...
RegisterServerEvent('qf_mdt:submitFine', function(data)
    ...
    if zPlayer then
        ...
        if data.jail and data.jail > 0 then
            ...
            TriggerEvent('esx_jtestailer:sendToJailPanelhype', tonumber(zPlayer.source), tonumber(data.jail * 60))
            MySQL.insert('INSERT INTO qf_mdt_jails (identifier, reason, fine, jail, date, officer) VALUES (?, ?, ?, ?, ?, ?)', {data.identifier, data.reason, data.fine, data.jail, os.time(), data.officer})
        else
            xPlayer.showNotification('Wystawiłeś mandat dla gracza [' .. zPlayer.source .. '] na ' .. data.fine .. ' $')
            zPlayer.showNotification('Otrzymałeś mandat od  [' .. xPlayer.source .. '] na ' .. data.fine .. ' $')
            MySQL.insert('INSERT INTO qf_mdt_fines (identifier, reason, fine, date, officer) VALUES (?, ?, ?, ?, ?)', {data.identifier, data.reason, data.fine, os.time(), data.officer})
        end
    else
