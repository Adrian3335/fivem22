    ...
    if xPlayer.getJob().grade_name == 'boss' then
        if xTarget then
            ...
            if jobname == 'police' or jobname == 'ambulance' or jobname == 'mechanic' or jobname == 'lssd' then
                ...
                if updateBadge then
                    -- Aktualizacja odznaki dla gracza
                    playerBadges[xTarget.source] = badge
                    return true
                end
            end
        else
...
exports('GetPlayerBadge', function(playerId)
    return playerBadges[playerId] or 0 -- Zwraca odznakę gracza lub 0, jeśli nie ma przypisanej odznaki
end)
