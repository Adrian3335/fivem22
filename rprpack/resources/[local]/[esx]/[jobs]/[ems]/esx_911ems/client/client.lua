...
RegisterCommand('911ems', function(source, args, rawCommand)
	TriggerEvent("chatMessage"," [911] " , {26, 83, 255},   "Zgłoszenie wysłane" )
	...
	local emergency = '911'
    TriggerServerEvent('esx_jobChatt:911',{
...
RegisterNetEvent('esx_jobChatt:911EmergencySend')
AddEventHandler('esx_jobChatt:911EmergencySend', function(messageFull)
	PlayerData = ESX.GetPlayerData()
...
RegisterNetEvent('esx_jobChatt:911Marker')
AddEventHandler('esx_jobChatt:911Marker', function(targetCoords, type)
	PlayerData = ESX.GetPlayerData()
	if PlayerData.job.name == 'ambulance' then
        local alpha = 250
