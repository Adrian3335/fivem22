...
local function startFueling(vehicle, isPump)
	...
	local price, moneyAmount, black_money
	local durability = 0
	...
	if isPump then
		...
		black_money = ox_inventory:Search(2, 'black_money')
	end
	...
	while isFueling do
		if isPump then
			...
			if price >= moneyAmount and price >= black_money then
				TriggerEvent("cocorp_progbar:client:cancel")
			    isFueling = false
				--lib.cancelProgress()
			end
		else
...
local function getPetrolCan(pumpCoord, refuel)
	...
	TriggerEvent("cocorp_progbar:client:progress", {
		name = "myciewiezienie",
