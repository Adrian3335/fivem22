...
ESX.RegisterCommand('setjob', {'best', 'superadmin', 'admin'}, function(xPlayer, args, showError)
	if xPlayer then
		if ESX.DoesJobExist(args.job, args.grade) then
			args.playerId.setJob(args.job, args.grade)
			-- exports['diablohub_logs']:SendLog(xPlayer.source, "Użyto komendy /setjob " .. args.playerId.source .. " " .. args.job .. " " .. args.grade, "job")
		else
...
ESX.RegisterCommand('car', 'admin', function(xPlayer, args, showError)
	...
	if playerVehicle and playerVehicle ~= 0 then
		...
		exports["dbl_carkeys"]:RemoveKey(xPlayer.source, plate)
	end
	...
	ESX.OneSync.SpawnVehicle(args.car, playerCoords, playerHeading, upgrades, function(networkId)
		if networkId then
			...
			exports["dbl_carkeys"]:AddKey(xPlayer.source, upgrades.plate)
		end
	end)
...
ESX.RegisterCommand({'cardel', 'dv'}, 'admin', function(xPlayer, args, showError)
	...
	if DoesEntityExist(PedVehicle) then
		local plate = GetVehicleNumberPlateText(PedVehicle, true)
		if type(plate) == "string" then
			plate = plate:gsub("%s$", "")
		end
