...
SetConvarServerInfo('Server stworzony przez ', 'XXXX')

local newPlayer = 'INSERT INTO `users` SET `accounts` = ?, `identifier` = ?, `discordId` = ?, `group` = ?'
local loadPlayer = 'SELECT `accounts`, `job`, `job_grade`, `group`, `position`, `inventory`, `skin`, `loadout`'

if Config.Multichar then
  newPlayer = newPlayer .. ', `firstname` = ?, `lastname` = ?, `dateofbirth` = ?, `sex` = ?, `height` = ?, `skin` = ?'
end
...
function createESXPlayer(identifier, playerId, data)
  ...
  for _, playerIdentifiersData in ipairs(GetPlayerIdentifiers(playerId)) do
    if playerIdentifiersData:match("discord") then
        discord = playerIdentifiersData:gsub("discord:", "")
    end
  end
  ...
  if not Config.Multichar then
    ...
    {json.encode(accounts), identifier, discord, defaultGroup, data.firstname, data.lastname, data.dateofbirth, data.sex, tonumber(data.height), json.encode(data.skin)}, function()
        loadESXPlayer(identifier, playerId, true)
      end)
  end
end
