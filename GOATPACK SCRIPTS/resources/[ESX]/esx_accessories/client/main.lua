...
RegisterNetEvent('niff_clothes:menu')
AddEventHandler('niff_clothes:menu', function(what)
	OpenAccessoryMenu()
end)
...
function OpenAccessoryMenu()
...

	{
		...
		elements = {
			...
			{label = 'Schowaj włosy pod czapke',value = 'hair_1', value2 = 'hair_2', lib = 'missheistdockssetup1hardhat@', anim  = 'put_on_hat', duration = -1, loop = 56},

			{label = 'Schowaj twarz pod maske',value = 'face', value2 = 'face2', lib = 'missheistdockssetup1hardhat@', anim  = 'put_on_hat' },

			{label = 'Ściągnij samą koszulkę',value = 'trs1', value2 = 'trs1', lib = 'clothingshirt', anim  = 'try_shirt_positive_d', duration = 2500, loop = 56},

			{label = 'Ściągnij sam tułów',value = 'tlow1', value2 = 'tlow1', lib = 'clothingshirt', anim  = 'try_shirt_positive_d', duration = 2500, loop = 56 },
	...
	function(data, menu)
		...
		TriggerEvent('skinchanger:getSkin', function(skin)
			...
			if skin.sex == 0 and (updatedSkin[currentData.value] ~= clearSkinMale[currentData.value] or currentData.value == 'tshirt_1' and updatedSkin[currentData.value] == 15 and updatedSkin['torso_1'] ~= 15) or currentData.value == 'trs1' or currentData.value == 'tlow1' then
				...
				if currentData.value == 'trs1' then
					...
					lattrs = skin.tshirt_1

					newSkin['tshirt_1'] = clearSkinMale["tshirt_1"]

					else 

						newSkin['tshirt_1'] = lattrs
				...
				if currentData.value == 'tlow1' then
					...
					if lattrss == nil then  
						...
						lattrss = skin.torso_1

					newSkin['torso_1'] = clearSkinMale["torso_1"]

					else 

						newSkin['torso_1'] = lattrss
			...
			elseif skin.sex == 1 and (updatedSkin[currentData.value] ~= clearSkinFemale[currentData.value] or currentData.value == 'tshirt_1' and updatedSkin[currentData.value] == 14 and updatedSkin['torso_1'] ~= 518) then
				...
				if currentData.value == 'trs1' then
					...
					newSkin['tshirt_1'] = lastSkin['tshirt_1']
...
RegisterCommand("naprawskin", function(source, args, rawCommand)

	if not fix then

		ESX.ShowNotification('~g~Rozpoczynasz naprawę skina ~w~wpisz komendę ponownie jeżeli skin już się naprawi!')

	else

		ESX.ShowNotification('~r~Kończysz naprawę skina! ~w~Ułatwiliśmy Ci życie :)')
...
RegisterCommand("fixpostac", function(source, args, rawCommand)
