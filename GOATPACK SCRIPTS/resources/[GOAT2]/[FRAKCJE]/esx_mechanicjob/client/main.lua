...
AddEventHandler('esx_mechanicjob:onWash', function()
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		...
		if DoesEntityExist(vehicle) then
			...
			local success = exports['goat_ui']:Progbar('Myjesz auto.', 3500)
			CreateThread(function()
				if success then
...
AddEventHandler('esx_mechanicjob:onCarokit', function()
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 5.0) then
		...
		if DoesEntityExist(vehicle) then
			...
			exports['goat_ui']:Progbar('Naprawiasz karoserię.', 30000)
			CreateThread(function()
				-- Citizen.Wait(10000)
...
local blips = {
     {title="Mechanik", colour=5, id=446, x = -216.0661, y = -1327.8917, z = 30.3505}
  }
...
AddEventHandler('esx_repairkit:onUse', function()
	...
	if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 3.1) then
		...
		if DoesEntityExist(vehicle) then			
			TaskStartScenarioInPlace(playerPed, "PROP_HUMAN_BUM_BIN", 0, true)
			exports['goat_ui']:Progbar('Naprawiasz silnik.', 20000)
			CreateThread(function()
...
RegisterNetEvent('need_mechanics:createUniform', function()
	exports["goat_multichar"]:openShopMenu(function()
	end, nil, "clothes", "Konfigurator Mechniaka", "Dostosuj strój swojej postaci.")
end)

RegisterNetEvent('goat_mechanicjob:openbossmenu', function()
	OpenBossMenu()
end)

RegisterNetEvent('goat_mechanicjob:client:giveBadge', function()
...
RegisterNetEvent('goat_mechanicjob:schowajauto', function()
    ESX.ShowNotification("Schowałeś pojazd")
    local vehicle = GetClosestVehicle(532.9270, -184.1393, 54.1585, 10.0, 0, 70)
    ESX.Game.DeleteVehicle(vehicle)
	exports['goat_carkeys']:RemoveKey(plate)
end)

RegisterNetEvent('goat_mechanicjob:vehiclepspawnermenu', function()
	OpenVehicleSpawnerMenu()
end)

RegisterNetEvent("goat_mechanicjob:odzyskajwfixkit", function()
	if przebrany and exports.ox_inventory:Search('count', 'toolfixkit') <= 0 and not zbieranie then
		zbieranie = true 
		FreezeEntityPosition(PlayerPedId(), true)
		local status = exports['goat_ui']:Progbar('Odzyskujesz części', 15000)
		FreezeEntityPosition(PlayerPedId(), false)
		zbieranie = false 
		ClearPedTasks(PlayerPedId())
		if status then
			TriggerServerEvent('goat_mechanicjob:serverTrigger', 'toolfixkit')
		end
	else
...
RegisterNetEvent("goat_mechanicjob:odzyskajwytrych", function()
	if przebrany and exports.ox_inventory:Search('count', 'toolwytrych') <= 0 and not zbieranie then
		...
		ClearPedTasks(PlayerPedId())
		if status then
			TriggerServerEvent('goat_mechanicjob:serverTrigger', 'toolwytrych')
		end
	else
		exports["goat_notify"]:Notify('warning', 'Powiadomienie', "Nie możesz tego zrobić.")
	end
...
RegisterNetEvent("goat_mechanicjob:sponge", function()
	if przebrany and exports.ox_inventory:Search('count', 'sponge') <= 0 and not zbieranie then
		zbieranie = true 
		FreezeEntityPosition(PlayerPedId(), true)
		local status = exports['goat_ui']:Progbar('Nalewasz wodę', 15000)
		FreezeEntityPosition(PlayerPedId(), false)
		zbieranie = false 
		ClearPedTasks(PlayerPedId())
		if status then
			TriggerServerEvent('goat_mechanicjob:serverTrigger', 'sponge')
		end
	else
		exports["goat_notify"]:Notify('warning', 'Powiadomienie', "Nie możesz tego zrobić.")
	end
...
RegisterNetEvent("goat_mechanicjob:odzyskajcarokit", function()
	if przebrany and exports.ox_inventory:Search('count', 'toolcarokit') <= 0 and not zbieranie then
		...
		ClearPedTasks(PlayerPedId())
		if status then
			TriggerServerEvent('goat_mechanicjob:serverTrigger', 'toolcarokit')
		end
	else
		exports["goat_notify"]:Notify('warning', 'Powiadomienie', "Nie możesz tego zrobić.")
	end
...
RegisterNetEvent("goat_mechanicjob:stworzwytrych", function()
	if not przerabianie then
		przerabianie = true 
		FreezeEntityPosition(PlayerPedId(), true)
		local status = exports['goat_ui']:Progbar('Tworzysz wytrych', 15000)
		FreezeEntityPosition(PlayerPedId(), false)
		przerabianie = false 
		ClearPedTasks(PlayerPedId())
		if status then
			TriggerServerEvent('goat_mechanicjob:serverTrigger', 'lockpick')
		end
	else
		exports["goat_notify"]:Notify('warning', 'Powiadomienie', "Nie możesz tego zrobić.")
	end
...
RegisterNetEvent("goat_mechanicjob:stworznaprawke", function()
	if not przerabianie then
		przerabianie = true 
		FreezeEntityPosition(PlayerPedId(), true)
		local status = exports['goat_ui']:Progbar('Tworzysz zestaw naprawczy', 15000)
		FreezeEntityPosition(PlayerPedId(), false)
		przerabianie = false 
		ClearPedTasks(PlayerPedId())
		if status then
			TriggerServerEvent('goat_mechanicjob:serverTrigger', 'fixkit')
		end
	else
		exports["goat_notify"]:Notify('warning', 'Powiadomienie', "Nie możesz tego zrobić.")
	end
...
RegisterNetEvent("goat_mechanicjob:stworzkaroserie", function()
	if not przerabianie then
		przerabianie = true 
		FreezeEntityPosition(PlayerPedId(), true)
		local status = exports['goat_ui']:Progbar('Tworzysz zestaw naprawczy karoserii', 15000)
		FreezeEntityPosition(PlayerPedId(), false)
		przerabianie = false 
		ClearPedTasks(PlayerPedId())
		if status then
			TriggerServerEvent('goat_mechanicjob:serverTrigger', 'carokit')
		end
	else
		exports["goat_notify"]:Notify('warning', 'Powiadomienie', "Nie możesz tego zrobić.")
	end
...
function CanOpenTuningMenu()
	return ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' and IsPedInAnyVehicle(ESX.PlayerData.ped) and Config.TuningZone.Zone:contains(GetEntityCoords(ESX.PlayerData.ped))
end

CreateThread(function()
	while GetResourceState("goat_base") ~= "started" do
		Wait(0)
	end
		
	exports['goat_base']:SpawnPed("heniek11", 'a_m_m_indian_01', vec4(-193.1915, -1293.6831, 30.3465, 272.11187744141), {
