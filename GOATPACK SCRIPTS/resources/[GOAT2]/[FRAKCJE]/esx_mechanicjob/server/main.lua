...
RegisterServerEvent('goat_mechanicjob:serverTrigger', function(name)
	local source = source
	...
	if xPlayer.job.name ~= 'mechanic' then 
	   ...
	   return exports["goat_notify"]:notify(source, "warning", "Uwaga!", "Wypierdalasz z serwera!")
	end
...
functions = {
	toolwytrych = function(xPlayer)
	   if xPlayer.getInventoryItem('toolwytrych').count >= 5 then
		  exports["goat_notify"]:notify(source, "warning", "Powiadomienie!", "Nie masz miejsca na wiecej części.")
	   else
	...
	sponge = function(xPlayer)
		if xPlayer.getInventoryItem('sponge').count >= 5 then
		   exports["goat_notify"]:notify(source, "warning", "Powiadomienie!", "Nie masz miejsca na wody.")
		else
	...
	sprzedaj = function(xPlayer)
	   if xPlayer.getInventoryItem('stol').count >= 10 then
		  ...
		  exports["goat_notify"]:notify(source, "warning", "Powiadomienie!", "Nie masz gotowych blatow.")
	   end
	end 
}
...
function sendToDiscord (name,message,color)
	...
	local webhook = 'https://discord.com/api/webhooks/946870556500844594/0GL-S_h0_DbwXCQOKOTqfc4zdR_QpOJ_XygEZ6SgLIwgnx058HkgkPOXYKX_dR_IwVK4'
