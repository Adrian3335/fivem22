AddEventHandler('fumoPolice:uniform', function(data)
    ...
    table.insert(categories, {label = "Konfiguracja", value = "shop"})
    table.insert(categories, {label = "LSPD", value = "job"})
    table.insert(categories, {label = "LSSD", value = "lssd"})
    table.insert(categories, {label = "Ubrania specjalne", value = "special"})
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom'..job, {
        ...
        elseif data.current.value == "shop" then
            ...
            exports["goat_multichar"]:openShopMenu(function()
                menu.close()
            end, nil, "clothes", "Konfigurator Policyjny", "Dostosuj mundur swojej postaci.")
