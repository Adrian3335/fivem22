            ...
            if ESX.UI.Inventory.Area.Check(3.0) then
                ESX.UI.Inventory.Area.Build(3.0, false, false, function(target, _, npc)
                    if target then
                        ...
                        TriggerServerEvent("goat_animations:requestSynced", target, item.keyword)
                    else
...
RegisterNetEvent("goat_animations:openMenu", function()
    if ESX.PlayerData.dead then
        return
    end
...
RegisterNetEvent("goat_animations:syncRequest", function(requester, emote)
    ...
    Citizen.CreateThread(function()
        local menu = ESX.UI.Menu.Open("default", GetCurrentResourceName(), "animations_sync_request_menu", {
            ...
            if data.current.value then
                TriggerServerEvent("goat_animations:syncAccepted", requester, emote)
            end
...
RegisterNetEvent("goat_animations:playSynced", function(emote, player)
    Functions.ClearTasks()
    Wait(300)
...
RegisterNetEvent("goat_animations:playSyncedSource", function(emote, player)
    Functions.ClearTasks()
    Wait(300)
