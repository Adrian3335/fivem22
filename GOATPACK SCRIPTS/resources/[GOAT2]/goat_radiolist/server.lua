...
RegisterNetEvent("pma-voice:setPlayerRadio", function(_radioChannel)
    ...
    TriggerClientEvent("goat_radiolist:connected", _source, radioChannel)
    ...
    if lastRadioChannel and lastRadioChannel ~= 0 and lastRadioChannel ~= radioChannel then
        ...
        if radios[lastRadioChannel] then
        ...
        if findUser then
            ...
            TriggerClientEvent("goat_radiolist:updateList", -1, lastRadioChannel, radios[lastRadioChannel])
        end
    end
    ...
    if radioChannel and radioChannel ~= 0 then
        ...
        if not findUser2 then
            local name = "[" .. _source .. "]"
            local nickname = xPlayer.get("radio-nick")
            if radioChannel <= 12 then
            ...
            TriggerClientEvent("goat_radiolist:updateList", -1, radioChannel, radios[radioChannel])
        end
    end
end)
...
RegisterNetEvent("pma-voice:setTalkingOnRadio", function(boolean)
    ...
    TriggerClientEvent("goat_radiolist:setTalkingOnRadio", -1, radioChannel, _source, boolean)
end)