...
function GetChannelList()
    ...
    for index,value in pairs(Channels) do 
        ...
        table.insert(menu_list, "Kanal #" .. index .. " (".. value.name ..")")
...
function BroadcastMenu() 
    ...
    local input = lib.inputDialog('Live na zywo', {'Nazwa kanalu:', 'Link do live:'})
...
function WebBrowserMenu()
    lib.hideMenu()
    local input = lib.inputDialog('Przegladarka', {'LINK:'})
...
function VideoMenu()
    lib.hideMenu()
    local input = lib.inputDialog('Odtwarzacz video', {'LINK:'})
    if input then 
        TriggerServerEvent("ptelevision:event", CURRENT_SCREEN, "ptv_status", {
            type = "play",
            url = input[1]
...
function VolumeMenu()
    lib.hideMenu()
    local input = lib.inputDialog('Glosnosc', {'Ustaw glosnosc (0-100):'})
...
function OpenTVMenu() 
    ...
    lib.registerMenu({
        ...
        options = {
            {label = 'Filmy', description = 'Odtwarzaj film lub transmisję na ekranie.'},
            {label = 'Przeglądarka internetowa', description = 'Dostęp do internetu przez telewizor.'},
            {label = 'Kanały TV', values = ChannelList.display, description = 'Kanały telewizyjne na żywo w San Andreas!', defaultIndex = ChannelList.current},
            {label = 'Interakcja z ekranem', description = 'Pozwala kontrolować elementy na ekranie.'},
            {label = 'Ustaw głośność', description = 'Ustawia głośność telewizora (dla ciebie).'},
            {label = 'Zamknij menu', close = true}
        }
