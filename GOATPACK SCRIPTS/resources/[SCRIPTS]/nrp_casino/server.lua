...
AddEventHandler("esx_slots:BetsAndMoney", function(bets)
    ...
    if xPlayer then
        ...
        if xItem.count < 10 then
            xPlayer.showNotification("Potrzebujesz conajmniej 10 żetonów aby zagrać!")
...
AddEventHandler("route68_kasyno:WymienZetony", function(count)
    ...
    if xPlayer then
        ...
        if xItem.count < count then
            xPlayer.showNotification("Nie masz tyle żetonów!")
        elseif xItem.count >= count then
            ...
            xPlayer.showNotification("Otrzymano $"..kwota.." w zamian za "..count.." żetonów.")
        end
    end
end)
...
AddEventHandler("route68_kasyno:KupZetony", function(count)
    ...
    if xPlayer then
        ...
        elseif kwota <= cash then
            ...
            xPlayer.showNotification("Otrzymałeś/aś "..count.." żetonów w zamian za $"..kwota..".")
        end
    end
end)
...
AddEventHandler("route68_kasyno:KupAlkohol", function(count, item)
    ...
    if xPlayer then
        ...
        local kwota = math.floor(count * 10)
        if kwota > cash then
           xPlayer.showNotification("Nie masz tyle pieniędzy!")
        elseif kwota <= cash then
            xPlayer.addInventoryItem(item, count)
            xPlayer.removeMoney(kwota)
            xPlayer.showNotification("Zakupiono "..count.." sztuk alkoholu za $"..count..".")
        end
    end
end)
...
AddEventHandler("route68_kasyno:getJoinChips", function()
    ...
    MySQL.Async.fetchAll('SELECT zetony FROM users WHERE @identifier=identifier', {
		...
		if result[1] then
            ...
            if zetony > 0 then
                xPlayer.showNotification("Zwrócono Ci "..tostring(zetony).." żetonów, ponieważ przed wyjściem Twoja postać grała w sloty.")
