...
function insertCoin(amount) {
  ...
  $.post('http://goat_casino/updateBets', JSON.stringify({
        bets: coins
      }));
}
...
function endWithWin(x, sound) {
  $.post('http://goat_casino/wygrana', JSON.stringify({
      win: x
    }));
...
function spin(timer) {
  ...
  for(var k in lines) {
    ...
    for (var x = 0; x < lines[k].length; x++) {
...
function togglePacanele(start, banuti) {
  if(start == true) {
    ...
    $.post("http://goat_casino/exitWith", JSON.stringify({
