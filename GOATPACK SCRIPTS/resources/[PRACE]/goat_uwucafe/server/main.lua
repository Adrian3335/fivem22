...
RegisterServerEvent('goat_uwucafe:serverTrigger', function(name)
    local source = source
...
functions = {
    zrobkawe = function(xPlayer)
        if xPlayer.getInventoryItem('coffee').count >= 3 then
            exports["goat_notify"]:notify(source, "warning", "Powiadomienie!", "Nie masz wolnych rąk na 3 kawy")
        else
            xPlayer.addInventoryItem('coffee', '1')
            exports["goat_notify"]:notify(source, "success", "Powiadomienie!", "Zrobiłeś kawę")
        end
    end,
    zbieranieskladnikow = function(xPlayer)
        if xPlayer.getInventoryItem('uwucafe_skladniki').count >= 1 then
            exports["goat_notify"]:notify(source, "warning", "Powiadomienie!", "Nie masz miejsca na więcej składników")
        else
            xPlayer.addInventoryItem('uwucafe_skladniki', '1')
            exports["goat_notify"]:notify(source, "success", "Powiadomienie!", "Przygotowałeś/aś składniki")
        end
    end,
    robiszciasto = function(xPlayer)
        if xPlayer.getInventoryItem('uwucafe_zestaw').count >= 5 then
            exports["goat_notify"]:notify(source, "warning", "Powiadomienie!", "Nie masz miejsca na więcej ciasta")
        else
    ...
    zabierasztowar = function(xPlayer)
        if xPlayer.getInventoryItem('towar').count >= 100 then
            exports["goat_notify"]:notify(source, "warning", "Powiadomienie!", "Nie masz miejsca na towar.")
        else
            xPlayer.addInventoryItem('towar', '100')
            exports["goat_notify"]:notify(source, "success", "Powiadomienie!", "Odebrałeś towar, wróc na firmę i odłoż na magazynie")
        end
    end,
    towarzabierz = function(xPlayer)
        if xPlayer.getInventoryItem('towar').count >= 100 then
           ...
           exports["goat_notify"]:notify(source, "success", "Powiadomienie!", "Odłożyłeś towar")
        else
           exports["goat_notify"]:notify(source, "warning", "Powiadomienie!", "Nie masz towaru")
        end
    end 
}

RegisterNetEvent('goat_uwucafe:propozycja', function(target, amount)
    local xPlayer = ESX.GetPlayerFromId(source)
    if xPlayer.job.name ~= 'kawiarnia' then return
    end
        TriggerClientEvent("goat_uwucafe:rachunek", target, source, amount)
end)
 
RegisterNetEvent('goat_uwucafe:wystawrachunek', function(target, amount, pay)
