...
RegisterNetEvent("goat_property:loadStash", function()
    local _source = source
    local identifier = ESX.GetPlayerFromId(_source).identifier
    exports.ox_inventory:RegisterStash("playerFreeProperty-" .. identifier, "Mieszkanie", 50, 250000, identifier, false, vec3(-816.56, 3469.93, -159.15))
end)

RegisterNetEvent("goat_property:saveOutfit", function(label, skin)
    local _source = source
...
RegisterNetEvent("goat_property:removeOutfit", function(label)
    local _source = source
    local identifier = ESX.GetPlayerFromId(_source).identifier
    TriggerEvent('esx_datastore:getDataStore', 'property', identifier, function(store)
...
ESX.RegisterServerCallback("goat_property:getPlayerDressing", function(source, cb)
    local _source = source
    local identifier = ESX.GetPlayerFromId(_source).identifier
    TriggerEvent('esx_datastore:getDataStore', 'property', identifier, function(store)
        local labels = {}
