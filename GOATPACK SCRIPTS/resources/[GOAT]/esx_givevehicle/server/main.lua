...
AddEventHandler('esx_giveownedcar:setVehicle', function (vehicleProps, playerID, vehicleType)
	...
	MySQL.Async.execute('INSERT INTO owned_vehicles (owner, plate, vehicle, vin, ssn) VALUES (@owner, @plate, @vehicle, @vin, @ssn)',
	{
...
AddEventHandler('esx_giveownedcar:setVehicle2', function (vehicleProps, playerID, vehicleType, job)
	local _source = playerID
	local xPlayer = ESX.GetPlayerFromId(_source)

	MySQL.Async.execute('INSERT INTO owned_vehicles (owner, plate, vehicle, vin, ssn, job) VALUES (@owner, @plate, @vehicle, @vin, @ssn, @job)',
	{
		['@owner']   = xPlayer.identifier,
		['@plate']   = vehicleProps.plate,
		['@vehicle'] = json.encode(vehicleProps),
