...
function updateLastNotes(src)
    ...
    for i = 1, #getLastCitizenNotes, 1 do
        ...
        table.insert(SERVER.lastNotes.citizen, {
            ...
            medic = getLastCitizenNotes[i].medic
        })
    end
...
ESX.RegisterServerCallback('qf_mdt:getData', function(source, cb, data)
    ...
    if data.type == 'citizen' then
        MySQL.single('SELECT identifier, firstname, lastname, dateofbirth, sex, NULL as avatar FROM users WHERE identifier = ? LIMIT 1', {data.identifier}, function(results)
            ...
            local fines = MySQL.query.await('SELECT * FROM ems_mdt_fines WHERE identifier = ? ORDER BY date DESC', {data.identifier})
...
RegisterServerEvent('qf_mdt:submitFineEMS', function(data)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    if not Config.Jobs[xPlayer.job.name] then
        return
    ...
    if zPlayer then
        ...
        if data.jail and data.jail > 0 then
            ...
            MySQL.insert('INSERT INTO qf_mdt_jails (identifier, reason, fine, jail, date, officer) VALUES (?, ?, ?, ?, ?, ?)', {data.identifier, data.reason, data.fine, data.jail, os.time(), data.medic})
        else
            ...
            MySQL.insert('INSERT INTO ems_mdt_fines (identifier, reason, fine, date, medic) VALUES (?, ?, ?, ?, ?)', {data.identifier, data.reason, data.fine, os.time(), data.medic})
        end
    else
