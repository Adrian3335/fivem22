...
-- Dodaj logi do sprawdzenia, czy event jest wywoływany
RegisterNetEvent('ricky-bossmenu:openMenu')
AddEventHandler('ricky-bossmenu:openMenu', function(job)
    print("Event 'ricky-bossmenu:openMenu' wywołany z job:", job)
    OpenBossMenu(job)
end)

exports.ox_target:addBoxZone({
    coords = vec3(618.5974, -18.1145, 90.4701),
    size = vec3(2, 2, 2),
    rotation = 64,
    drawSprite = true,
    options = {
        {
            ...
            onSelect = function()
                ...
                TriggerEvent('ricky-bossmenu:openMenu', job)
            end,
        }
    }
})
